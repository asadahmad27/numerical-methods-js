...
import {
    initialMatrix15 as initialMatrix, initialInputColumn12 as initialInputColumn,
    generateGridCallback, createNewColumn, createNewRow, gridTo2DArray
} from "../../matrix_utils";
    ...
    let graphCallback = (calculator, currentResult) => {
        for (let i = 0; i < matrixSize; i++) {
            calculator.current.setExpression({
                id: `Eigenvalue ${i + 1}`, color: Desmos.Colors.RED, pointStyle: Desmos.Styles.POINT, label: `Eigenvalue ${i + 1}`, showLabel: true, latex:
                    `(${formatLatex(realEigenValues[i])}, ${formatLatex(imaginaryEigenvalues[i])})`
            });
            calculator.current.setExpression({
                id: `Disk ${i + 1}`, color: Desmos.Colors.BLUE, latex: String.raw
